<template>
    <div class="container">
        ...
        <p>生成随机字符图标：</p>
        <p>{{emojiText}}
            {{ emojiText.key }}:{{ emojiText.emoji }}</p>
            &#x1F635;&#x26D1;
...
export default {
    ...
    mounted() {
        ...
        console.log(Object.getOwnPropertyNames(emoji.emoji).length);
        console.log(emoji.unemojify('I ❤️ 🍕'));

        console.log('U+26D1'.replace('U+', '&#x'));
        console.log(this.stringToHex('1F627').replace(/,/g, ''));

        setInterval(() => {
            self.emojiText = emoji.random();
        }, 1500);
